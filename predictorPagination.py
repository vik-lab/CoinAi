...
API_KEY = "CG-ZPgqfseWx832pLEVRmEsi26B"

# Function to retrieve paginated predictions
def get_paginated_predictions(predictions, page=1, per_page=10):
    start_index = (page - 1) * per_page
    end_index = start_index + per_page
    paginated_predictions = dict(list(predictions.items())[start_index:end_index])
...
def index():
    ...
    if request.method == 'POST':
        ...
        if coin_id in coins:
            ...
            paginated_predictions = get_paginated_predictions(predictions, page=page)
            return render_template('index.html', predictions=paginated_predictions, coins=coins, coin_id=coin_id, days=days)
        else:
...
def get_market_chart(coin_id, days):
    ...
    print("OHLC Data:", ohlc_data)  # Print JSON response
    ...
    print("History Data:", history_data)  # Print JSON response
    ...
    print("Market Data:", market_data)  # Print JSON response
